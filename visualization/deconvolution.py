'''
Deconvolution library for visualizing Keras CNN

Author: github.com/himanshurawlani
Cited: 6/20/19
Visualizing Interpretability for Convolutional Neural Networks: Visualizing Using Deconvnets
Sourced from:
...
def visualize_all_layers(model, data, layer_name='predictions', visualize_mode='all'):
    ...
    for layer in model.layers:
        if isinstance(layer, Convolution2D):
            ...
            print('Cannot handle layer type')
    ...
    layers_to_visualize = [x for x, y in enumerate(deconv_layers) if y[0] in model_layers]
    layers_to_visualize.reverse()
    layers_to_visualize.pop()
    print('Layers to visualize: ', layers_to_visualize)
    ...
    for i in layers_to_visualize:
        ...
        print('Deconv layer: ', deconv_layers[i][0])
        print('Top filters: ', top_filters)

        for feature_to_visualize, sum_value in top_filters:
            assert output.ndim in (2, 4)
            if output.ndim == 2:
